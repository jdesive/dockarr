...
import {COMPOSE_FILES, DOCKARR_NETWORK_COMPOSE, DockerComposeFile} from "./compose-files.models";
...
export const DOCKARR_VERSION = "0.0.1";
...
async function createStack(name: string, options: any) {
    ...
    if (options.services === undefined) {
        ...
        for (let string of selectionArray) {
            if (!isNaN(Number(string))) { // If string is actually number
                if ((COMPOSE_FILES.length - 1) >= Number(string)) {
    ...
    for (let dockerComposeFile of servicesToDeploy) {
        ...
        fileContentStr = fileContentStr.replace(/\$DockerSock/g, options.dockerSockFile);
    ...
    const doc = new YAML.Document({version: "3.9"});
    doc.commentBefore = 'Generated with <3 by Dockarr on ' + new Date();
    doc.comment = 'EOF';
    doc.add(doc.createPair("name", name));
    doc.add(doc.createPair("services", composeManifest));
    doc.add(doc.createPair("networks", DOCKARR_NETWORK_COMPOSE));
...
async function run() {
    ...
    dockarrCommand.command("start")
        ...
        .option('-s, --service [service]', "Only stop one service in the deployment")
        .action(async (name, options) => {
            const deploymentDir = Path.join(dockarrDir, name);
            console.log(chalk.blue('Starting deployment...'));

            if (options.service !== undefined) {
                compose.upMany(options.service.split(','), {cwd: deploymentDir, config: [name + ".yaml"], log: true})
                    .then(
                        () => {
        ...
        .action(async (name, options) => {
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i].name === name) {
                    if (options.service !== undefined) {
                        await compose.stopMany({cwd: deploymentDir, config: [name + ".yaml"], log: true}, options.service.split(',')).then(
                            () => {
                                console.log(chalk.green('Finished!'));
                            },
                            err => {
    ...
    dockarrCommand.command("restart")
        ...
        .option('-s, --service [service]', "Only restart one service in the deployment")
        .action(async (name, options) => {
            const deploymentDir = Path.join(dockarrDir, name);
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i].name === name) {
                    if (options.service !== undefined) {
                        await compose.restartMany(options.service.split(','), {cwd: deploymentDir, config: [name + ".yaml"], log: true}).then(
                            () => {
                                console.log(chalk.green('Finished!'));
                            },
                            err => {
